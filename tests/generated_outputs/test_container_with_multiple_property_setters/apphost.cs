...
var cache = builder.AddConnectionString(name: "cache", environmentVariableName: (string?)null);
...
mycontainer.WithImage(image: "nginx:alpine", tag: (string?)null);
...
mycontainer.WithReference(source: cache, connectionName: (string?)null, optional: false);
mycontainer.WithHttpEndpoint(port: 8080, targetPort: 80, name: "http", env: "env", isProxied: false);
mycontainer.WithHttpEndpoint(port: 9090, targetPort: null, name: "http-alt", env: (string?)null, isProxied: true);
